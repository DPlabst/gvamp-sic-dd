...
for modstr = ["ASK-0.2", "ASK-1"]
    for M = [16]
        [filename, vver] = gvamp( ...
            ...
            Ptx = '-16:2:30', ... % Transmit powers in dB, e.g.,'-10:2:50' (mind the SNR definition)
            ...
            nu = [1, 0], ... % Noise variances of [optical white CSCG, electrical real WGN].
            RxBW = 1, ... % [0/1]: Apply PD filter to filter out-of-band noise 
            ADCfilt = 0, ... %[0/1]: Apply ADC filter with bandwidth [-B, +B]
            outdencfg = {[0, 0], 'SPA'}, ... % Use scaled [nu(1), nu(2)] in output denoiser, calc. generalized Xi2 via method {SPA, Davies} when electrical noise > 0
            dampcfgStepW = [3, 3, 1], ... [minWindow, maxWindow, [0/1]:reduce step window with increasing SIC stage]
            annealcfg = {[100, 1], [0.36, 0.19, 1], [0]}, ... % Cell-array {[n_it_thresh, reduce threshold vs. SIC stage], [alpha, beta, minAnneal], [anneal if iter > switchIterThresh]}
            cons = 0, ... % Console output
            save_file = 1, ... % Save results in MAT and TXT file
            VAMP_dbg_mode = 0, ... % [0,1,2]; 0=none, 1=plot rate trace, 2=plot internal variance vs. actual variances
